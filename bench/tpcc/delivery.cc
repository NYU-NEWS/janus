...
void TpccPiece::RegDelivery() {
  ...
  BEGIN_PIE(TPCC_DELIVERY, TPCC_DELIVERY_0, DF_REAL) {
    ...
    if (rs.has_next()) {
      r = rs.next();
//      TPL_KISS_ROW(r);
      dtxn->ReadColumn(r, TPCC_COL_NEW_ORDER_NO_W_ID, &o_id, TXN_DEFERRED);
...
//    mdb::MultiBlob mbl(4), mbh(4);
    ...
    mdb::ResultSet rs_ol = dtxn->QueryIn(dtxn->GetTable(TPCC_TB_ORDER_LINE),
                                         mbl,
                                         mbh,
                                         mdb::ORD_ASC,
                                         RS_ORDER_LINE);
    ...
    while (i < row_list.size()) {
      row_ol = row_list[i++];
      Value buf(0.0);
      dtxn->ReadColumn(row_ol, TPCC_COL_ORDER_LINE_OL_AMOUNT,
                       &buf, TXN_DEFERRED); // read ol_amount
      ol_amount_buf += buf.get_double();
      dtxn->WriteColumn(row_ol, TPCC_COL_ORDER_LINE_OL_DELIVERY_D,
...
//    verify(cmd.input.size() == 4);
    ...
    mb[0] = output[TPCC_VAR_C_ID].get_blob();
    ...
    buf.set_double(buf.get_double() + ol_amount_buf);
