...
void TpccDistChopper::new_order_shard(const char *tb, const std::vector<Value> &input, uint32_t &site, int cnt) {
    MultiValue mv;
...
void TpccDistChopper::payment_shard(const char *tb, const std::vector<Value> &input, uint32_t &site) {
    MultiValue mv;
...
void TpccDistChopper::order_status_shard(const char *tb, const std::vector<mdb::Value> &input, uint32_t &site) {
    MultiValue mv;
...
void TpccDistChopper::delivery_shard(const char *tb, const std::vector<mdb::Value> &input, uint32_t &site, int cnt) {
    MultiValue mv;
...
void TpccDistChopper::stock_level_shard(const char *tb, const std::vector<mdb::Value> &input, uint32_t &site) {
    MultiValue mv;
    if (tb == TPCC_TB_DISTRICT
     || tb == TPCC_TB_ORDER_LINE)
        // based on d_id & w_id
