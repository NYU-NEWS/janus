...
bool SchedulerOcc::DoPrepare(txnid_t tx_id) {
  ...
  if (!txn->version_check()) {
    Log_debug("txn: %llx occ validation failed.", (int64_t)tx_id);
    tx_box->result_prepare = false;
    return false;
  } else {
    // now lock the commit
    for (auto &it : txn->ver_check_read_) {
      ...
      if (cc_it == TxnMgr::rl_conflict_count_.end())
        ...
        txn->locks_.clear();
        Log_debug("txn: %llx occ read locks failed.", (int64_t)tx_id);
        tx_box->result_prepare = false;
        return false;
      }
    ...
    for (auto &it : txn->updates_) {
      ...
      if (cc_it == TxnMgr::wl_conflict_count_.end())
        ...
        txn->locks_.clear();
        Log_debug("txn: %llx occ write locks failed.", (int64_t)tx_id);
        tx_box->result_prepare = false;
        return false;
      }
      insert_into_map(txn->locks_, row, -1);
