...
map<std::pair<base::i32, base::i32>,
    TxnRegistry::txn_handler_defer_pair_t> TxnRegistry::all_;
//map<std::pair<base::i32, base::i32>, TxnRegistry::LockSetOracle> TxnRegistry::lck_oracle_;

map<i64, std::vector<TxnRegistry::DeferredRequest>> TxnRegistry::deferred_reqs_;
...
void TxnRegistry::exe_deptran_start(
    ...
    verify(pv && tv);
    auto txn_handler_pair = get(header.t_type, header.p_type);
...
void TxnRegistry::exe_deptran_start_ro(
    const RequestHeader &header,
    const std::vector<mdb::Value> &input,
    std::vector<mdb::Value> &output,
...
void TxnRegistry::exe_deptran_finish(i64 tid,
