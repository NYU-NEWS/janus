...
public:
    ...
    std::vector<uint32_t> sharding_;     /** which server to which piece */
    std::vector<int> status_; // -1 waiting; 0 ready; 1 ongoing; 2 finished;
    std::set<int32_t> proxies_;   /** server involved*/

    int n_start_sent_ = 0;
    int n_pieces_ = 0;
    int n_started_ = 0; /** finished pieces counting */
    int n_prepared_ = 0;
    int n_finished_ = 0;
    ...
    virtual int batch_next_piece(BatchRequestHeader *batch_header, std::vector<mdb::Value> &input, int32_t &server_id, std::vector<int> &pi, Coordinator *coo);
