...
void Config::init_sharding(const char *filename) {
    ...
    size_t mode_str_i = 0;
    while (mode_str_i < mode_str.size()) {
    ...
    // get benchmark
    std::string benchmark_name = pt.get<std::string>("benchmark.<xmlattr>.name");
    if (benchmark_name == "tpca") {
        benchmark_ = TPCA;
    }
    else if (benchmark_name == "tpcc") {
        benchmark_ = TPCC;
    }
    else if (benchmark_name == "tpcc_dist_part") {
        benchmark_ = TPCC_DIST_PART;
    }
    else if (benchmark_name == "tpcc_real_dist_part") {
        benchmark_ = TPCC_REAL_DIST_PART;
    }
    else if (benchmark_name == "rw_benchmark") {
        benchmark_ = RW_BENCHMARK;
    }
    else if (benchmark_name == "micro_bench") {
        benchmark_ = MICRO_BENCH;
    }
    else {
        Log_error("No implementation for benchmark: %s", benchmark_name.c_str());
        verify(0);
    }
    ...
    concurrent_txn_ = pt.get<unsigned int>("benchmark.<xmlattr>.concurrent_txn", 1);
    batch_start_ = pt.get<bool>("benchmark.<xmlattr>.batch_start", false);

    std::string txn_weight_str = pt.get<std::string>("benchmark.<xmlattr>.txn_weight", "");
    size_t txn_weight_str_i = 0, end_txn_weight_str_i;
    char *txn_weight_str_endptr;
    while (string::npos != (end_txn_weight_str_i = txn_weight_str.find(':', txn_weight_str_i))) {
        double d = strtod(txn_weight_str.c_str() + txn_weight_str_i, &txn_weight_str_endptr);
        verify(txn_weight_str_endptr == txn_weight_str.c_str() + end_txn_weight_str_i);
    ...
    BOOST_FOREACH(boost::property_tree::ptree::value_type const &value, 
        ...
        if (value.first == "client") {
            ...
            if (pos == std::string::npos) { // one client id
                ...
                for (unsigned int mark_index = start_cid; mark_index <= end_cid; mark_index++) {
    ...
    BOOST_FOREACH(boost::property_tree::ptree::value_type const &value, 
        ...
        if (value.first == "table") {
            ...
            std::string method = value.second.get<std::string>("<xmlattr>.shard_method");

            unsigned long long int records = value.second.get<unsigned long long int>("<xmlattr>.records");
            ...
            if (all_site) {
                ...
                tb_info.site_id = (unsigned int *)malloc(sizeof(unsigned int) * num_site_);
                ...
                BOOST_FOREACH(boost::property_tree::ptree::value_type const &site_value, value.second.get_child("site")) {
                ...
                tb_info.site_id = (unsigned int *)malloc(sizeof(unsigned int) * num_site_buf);
                verify(tb_info.site_id != NULL);
                unsigned int j = 0;
                for (unsigned int i = 0; i < num_site_; i++)
                    if (site_buf[i] == 1)
            ...
            mdb::symbol_t symbol;
            std::string symbol_str = value.second.get<std::string>("<xmlattr>.type", "sorted");
            if (symbol_str == "sorted") {
                symbol = mdb::TBL_SORTED;
            }
            else if (symbol_str == "unsorted") {
                symbol = mdb::TBL_UNSORTED;
            }
            else if (symbol_str == "snapshot") {
                symbol = mdb::TBL_SNAPSHOT;
            }
            else {
                Log_fatal("Unknown table type: %s", symbol_str.c_str());
                verify(0);
            }
            tb_info.symbol = symbol;

            // set tb_info.columns
            BOOST_FOREACH(boost::property_tree::ptree::value_type const &column, value.second.get_child("schema")) {
                if (column.first == "column") {
                    ...
                    std::string c_foreign = column.second.get<std::string>("<xmlattr>.foreign", "");
                    ...
                    if (c_foreign.size() > 0) {
                        ...
                        std::map<std::string, Sharding::tb_info_t>::iterator ftb_it = Sharding::sharding_s->tb_info_.find(foreign_tb);
                        verify(ftb_it != Sharding::sharding_s->tb_info_.end());
                        foreign_key_tb = &(ftb_it->second);

                        std::vector<Sharding::column_t>::iterator fc_it = ftb_it->second.columns.begin();
                    ...
                    tb_info.columns.push_back(Sharding::column_t(c_v_type, c_name, c_primary, foreign_key_tb, foreign_key_column));
                }
            }
    ...
    if (benchmark_ == TPCC_REAL_DIST_PART) {
        i32 n_w_id = (i32)(Sharding::sharding_s->tb_info_[std::string(TPCC_TB_WAREHOUSE)].num_records);
        i32 n_d_id = (i32)(num_site_ * Sharding::sharding_s->tb_info_[std::string(TPCC_TB_DISTRICT)].num_records / n_w_id);
        i32 d_id = 0, w_id = 0;
        ...
        i32 n_i_id = (i32)(Sharding::sharding_s->tb_info_[std::string(TPCC_TB_ITEM)].num_records);
        i32 i_id = 0;
