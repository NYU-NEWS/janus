...
void TpccPiece::reg_new_order() {
    TxnRegistry::reg(TPCC_NEW_ORDER,
            ...
            if (TxnRunner::get_running_mode() == MODE_2PL
                ...
                std::function<void(void)> succ_callback = TxnRunner::
                    get_2pl_succ_callback(header, input, input_size, res, ps);
                std::function<void(void)> fail_callback = TxnRunner::
...
//            });
    ...
    TxnRegistry::reg(
            ...
            [] (const RequestHeader &header, 
                ...
                if (TxnRunner::get_running_mode() == MODE_2PL
                    && output_size == NULL) {
                    TxnRunner::get_2pl_proceed_callback(header, input,
                        input_size, res)();
                    return;
                }
    ...
    TxnRegistry::reg(
            ...
            [] (const RequestHeader &header, const Value *input, i32 input_size, i32 *res, Value *output, i32 *output_size, row_map_t *row_map, Vertex<PieInfo> *pv, Vertex<TxnInfo> *tv, std::vector<TxnInfo *> *conflict_txns) {
                ...
                if (TxnRunner::get_running_mode() == MODE_2PL
                    ...
                    std::function<void(void)> succ_callback = TxnRunner::
                        get_2pl_succ_callback(header, input, input_size, res,
                                ps, func);
                    std::function<void(void)> fail_callback = TxnRunner::
                        get_2pl_fail_callback(header, res, ps);
