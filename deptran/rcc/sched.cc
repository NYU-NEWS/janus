...
int RccSched::OnFinishRequest(cmdid_t cmd_id,
  ...
  waitlist_.push_back(graph.vertexes());
...
int RccSched::OnInquiryRequest(cmdid_t cmd_id,
  ...
  DragonBall *ball = new DragonBall(2, [cmd_id, callback, graph] () {
...
void RccSched::CheckWaitlist() {
  for (Vertex<TxnInfo> *v : wait_list_) {
    TxnInfo& tinfo = *(v->data_);
    if (CheckAllAncFinished(v)) {
...
void RccSched::send_ask_req(Vertex<TxnInfo> *av) {
