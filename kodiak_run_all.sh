...
IFS=',' read -a ALL_NODES_TMP <<<"${EMULAB_LIST}"

ALL_NODES=( )
for pro_node in ${ALL_NODES_TMP[@]}
do
    if [ ${pro_node} == 'h0' ]
    then
        echo "RUNNING ON ${pro_node}"
    else
        pro_node=${pro_node}".rococo.Spartan"
        ping -c 1 ${pro_node}
        if [ $? -eq 0 ]
        then
            chret=`ssh ${pro_node} "mount | grep rcc_log | grep sdb | wc -l"`
            if [ $? -eq 0 ]
            then
                if [ ${chret} -eq 1 ]
                then
...
# >>>>>>>> BENCHMARK >>>>>>>>
# benchmarks to run
#BENCHS=( tpcc_mix_10 tpcc_mix_1 )
BENCHS=( tpcc_mix_10 tpcc_no_10 )
#BENCHS=( ori_tpcc_no ori_tpcc_mix )
#BENCHS=( tpcc_no_10 )
...
# benchmark log
TIME=`date +"%H.%M.%S"`
CONF_XML="tmp_conf${TIME}.xml"
BENCHMARK_LOGGING_FILE="benchmark${TIME}.log"
RUN_LOGGING_DIR="log${TIME}"
...
#SCALE_BENCHS=( tpcc_mix_10 tpcc_w_mix_10 )
...
NULL_RPC_MIN_CLIENTS=55
NULL_RPC_MAX_CLIENTS=65

# kv starts from MICRO_BENCH_MIN_CLIENTS to MICRO_BENCH_MAX_CLIENTS
MICRO_BENCH_MIN_CLIENTS=40
MICRO_BENCH_MAX_CLIENTS=50

# for micro bench 3-piece txn
MICRO_BENCH_MIN_COOS=25
MICRO_BENCH_MAX_COOS=40

# for micro bench 3-piece txn
MICRO_BENCH_NUM_SERVS=3

MICRO_MODES=( deptran occ 2pl_wound_die none )
# <<<<<<<< MICRO BENCH <<<<<<<<
...
{
    local node_index=0
    local num_nodes=${#CLIENT_MACHINES[@]}
    for node_index in $(seq 0 $((${num_nodes}-1)))
    do
        local swap_index=$((${RANDOM}%${num_nodes}))
        local swap_tmp=${CLIENT_MACHINES[${node_index}]}
        CLIENT_MACHINES[${node_index}]=${CLIENT_MACHINES[${swap_index}]}
        CLIENT_MACHINES[${swap_index}]=${swap_tmp}
    done

    local node_index=0
    local num_nodes=${#SERV_MACHINES[@]}
    for node_index in $(seq 0 $((${num_nodes}-1)))
    do
        local swap_index=$((${RANDOM}%${num_nodes}))
        local swap_tmp=${SERV_MACHINES[${node_index}]}
        SERV_MACHINES[${node_index}]=${SERV_MACHINES[${swap_index}]}
        SERV_MACHINES[${swap_index}]=${swap_tmp}
    done
}
...
do_scaling_bench()
{
    ...
    do
        ...
        do
            ...
            local figure_gen_file=scripts/${bench}_${l_f_name}_figure_gen_kodiak.py
            cp -f template/scale_figure_gen_template.py ${figure_gen_file}
            sed -i "s/__BENCH_NAME__/kodiak\/${bench}_${l_f_name}/g" ${figure_gen_file}
            ...
            do
                ...
                do
                    ...
                    do
                        ...
                        make_bench_config ${template_file} ${mode} ${num_servs} ${total_num_coos} ${num_servs} >${CONF_XML}
                        ...
                        done < <(./run.py -T ${TASKSET} -f ${CONF_XML} -d ${BENCH_DURATION} -P ${CTRL_PORT} ${l_logging} -x "${interested_txn}" -l ${RUN_LOGGING_DIR} )
                    ...
                    then
                        ...
                        cp -f ${CONF_XML} failed_xml/${bench}/${mode}_${num_serv}servs_${num_coos}coos.xml
                    else
        ...
        do
            ...
            do
                ...
                then
                    ...
                    # backup xml file
                    mkdir -p failed_xml/${bench}
                    cp -f ${CONF_XML} failed_xml/${bench}/${mode}_${num_coos}coos.xml
                else
                    echo "============== SUCCESS ===============" >>${BENCHMARK_LOGGING_FILE}
                    echo "TRIED         :   ${tried}" >>${BENCHMARK_LOGGING_FILE}
                    echo "BENCHMARK     :   ${bench}" >>${BENCHMARK_LOGGING_FILE}
...
do_null_rpc_bench()
{
    ...
    local num_c=0
    local num_rpc_c=( 1 )
    for element in $(seq ${NULL_RPC_MIN_CLIENTS} ${NULL_RPC_MAX_CLIENTS})
    do
        num_rpc_c[${#num_rpc_c[@]}]=${element}
    done
    for num_c in ${num_rpc_c[@]}
    do
        local l_qps=0
        local l_50_lt=0
        local l_90_lt=0
        ...
        while [ ${tried} -lt ${MAX_RETRY} ]
        do
            ssh -nf ${SERV_MACHINES[0]} "cd ${FULL_PATH} && ./build/rpc_microbench -s 0.0.0.0:${SERV_START_PORT} -b 0 -w 1 -e 1 1>/dev/null 2>/dev/null &"
...
do_deptran_rpc_bench()
{
    ...
    do
        ...
        # PREPARE TO DO BENCHMARK
        make_bench_config ${template_file} ${mode} 3 ${num_coos} 3 >${CONF_XML}
        ...
        done < <(./run.py -T ${TASKSET} -f ${CONF_XML} -d ${BENCH_DURATION} -P ${CTRL_PORT} -x "${interested_txn}" -l ${RUN_LOGGING_DIR} )
        ...
        then
            # success
            sed -i "s/__null_rpc\.latency_50__/${l_latency_50}/g" ${figure_gen_file}
            sed -i "s/__null_rpc\.latency_90__/${l_latency_90}/g" ${figure_gen_file}
            sed -i "s/__null_rpc\.latency_99__/${l_latency_99}/g" ${figure_gen_file}
            sed -i "s/__null_rpc\.latency_999__/${l_latency_999}/g" ${figure_gen_file}
            sed -i "s/__null_rpc\.latency_999__/${l_latency_999}/g" ${figure_gen_file}
    ...
    then
        ...
        echo "============== FAILURE ===============" >>${BENCHMARK_LOGGING_FILE}
        echo "BENCH FAILED FOR ${MAX_RETRY} TIMES" >>${BENCHMARK_LOGGING_FILE}
        echo "BENCHMARK     :   micro_bench (null rpc latency)" >>${BENCHMARK_LOGGING_FILE}
        echo "INTERESTED TXN:   ${interested_txn}" >>${BENCHMARK_LOGGING_FILE}
        ...
        sed -i "s/__null_rpc\.latency_50__/-1, __null_rpc\.latency_50__/g" ${figure_gen_file}
        sed -i "s/__null_rpc\.latency_90__/-1, __null_rpc\.latency_90__/g" ${figure_gen_file}
        sed -i "s/__null_rpc\.latency_99__/-1, __null_rpc\.latency_99__/g" ${figure_gen_file}
        sed -i "s/__null_rpc\.latency_999__/-1, __null_rpc\.latency_999__/g" ${figure_gen_file}

        # backup xml file
        mkdir -p failed_xml/micro_kv_none
        cp -f ${CONF_XML} failed_xml/micro_kv_none/${interested_txn}_lt.xml
    else
        echo "============== SUCCESS ===============" >>${BENCHMARK_LOGGING_FILE}
        echo "TRIED         :   ${tried}" >>${BENCHMARK_LOGGING_FILE}
        echo "BENCHMARK     :   micro_bench (null rpc latency)" >>${BENCHMARK_LOGGING_FILE}
    ...
    for num_coos in $(seq ${NULL_RPC_MIN_CLIENTS} ${NULL_RPC_MAX_CLIENTS})
    do
        # TRY TO FINISH A BENCH, TILL MAX_RETRY
        local this_start_time=`date +"%H:%M:%S"`
        local s_time_in_s=`date +"%s"`
        ...
        do
            ...
            then
                # success
                sed -i "s/__null_rpc\.qps__/${l_tps}, __null_rpc\.qps__/g" ${figure_gen_file}
                sed -i "s/__null_rpc\.cpu__/${l_cpu_info}, __null_rpc\.cpu__/g" ${figure_gen_file}
        ...
        then
            ...
            sed -i "s/__null_rpc\.qps__/-1, __null_rpc\.qps__/g" ${figure_gen_file}
            sed -i "s/__null_rpc\.cpu__/-1, __null_rpc\.cpu__/g" ${figure_gen_file}

            # backup xml file
            mkdir -p failed_xml/micro_kv_none
            cp -f ${CONF_XML} failed_xml/micro_kv_none/${num_coos}coos.xml
        else
            echo "============== SUCCESS ===============" >>${BENCHMARK_LOGGING_FILE}
            echo "TRIED         :   ${tried}" >>${BENCHMARK_LOGGING_FILE}
            echo "BENCHMARK     :   micro_bench (null rpc)" >>${BENCHMARK_LOGGING_FILE}
        ...
        sed -i "s/__null_rpc\.num_coos__/${num_coos}, __null_rpc\.num_coos__/g" ${figure_gen_file}

    done


    sed -i "s/, __null_rpc\.qps__//g" ${figure_gen_file}
    sed -i "s/, __null_rpc\.cpu__//g" ${figure_gen_file}
    sed -i "s/, __null_rpc\.num_coos__//g" ${figure_gen_file}
...
do_kv_bench()
{
    ...
    do
        ...
        do
            ...
            then
                ...
                sed -i "s/__kv_none\.cpu__/${l_cpu_info}, __kv_none\.cpu__/g" ${figure_gen_file}
        ...
        then
            ...
            sed -i "s/__kv_none\.cpu__/-1, __kv_none\.cpu__/g" ${figure_gen_file}
    ...
    sed -i "s/, __kv_none\.cpu__//g" ${figure_gen_file}
...
do_micro_txn_bench()
{
    ...
    do
        ...
        do
            ...
            # PREPARE TO DO BENCHMARK
            make_bench_config ${template_file} ${mode} ${MICRO_BENCH_NUM_SERVS} ${num_coos} ${MICRO_BENCH_NUM_SERVS} >${CONF_XML}
        ...
        then
            ...
            cp -f ${CONF_XML} failed_xml/micro_bench/${mode}_${interested_txn}_lt.xml
        else
            echo "============== SUCCESS ===============" >>${BENCHMARK_LOGGING_FILE}
            echo "TRIED         :   ${tried}" >>${BENCHMARK_LOGGING_FILE}
            echo "BENCHMARK     :   micro_bench (4-piece txn latency)" >>${BENCHMARK_LOGGING_FILE}
    ...
    do
        ...
        do
            ...
            then
                ...
                # backup xml file
                mkdir -p failed_xml/micro_bench
                cp -f ${CONF_XML} failed_xml/micro_bench/${mode}_${num_coos}coos.xml
            else
                echo "============== SUCCESS ===============" >>${BENCHMARK_LOGGING_FILE}
                echo "TRIED         :   ${tried}" >>${BENCHMARK_LOGGING_FILE}
                echo "BENCHMARK     :   micro_bench (4-piece txn)" >>${BENCHMARK_LOGGING_FILE}
...
do_micro_bench()
{
    local figure_gen_file=scripts/micro_bench_figure_gen_kodiak.py
    cp -f template/micro_bench_figure_gen_template.py ${figure_gen_file}

    #do_null_rpc_bench ${figure_gen_file}

    do_deptran_rpc_bench ${figure_gen_file}
...
start_bench() {
    ...
    if [ ${scale} -eq 1 ]
    then
        local l_num_nodes=${#ALL_NODES[@]}
        local node_index=0

        SERV_MACHINES=( )
        for node_index in $(seq 0 $((${l_num_nodes}/2-1)))
        do
            SERV_MACHINES[${node_index}]=${ALL_NODES[${node_index}]}${USE_IB}
        done

        CLIENT_MACHINES=( )
        for node_index in $(seq 0 $((${l_num_nodes}/2-1)))
        do
            CLIENT_MACHINES[${node_index}]=${ALL_NODES[$((${node_index}+${l_num_nodes}/2))]}${USE_IB}
        done
        ...
        #    exit 2
        #fi
        local coos_needed=$((${NUM_SERVS}*${MAX_COO_SCALE}*${COOS_COARSE_UNIT}))
        local coos=$((${COOS_PER_CLIENT}*${MAX_CLIENT_PROCESSES}*${#CLIENT_MACHINES[@]}))
    ...
    then
        ...
        local l_num_servs=$((${NUM_SERVS}/${SERVS_PER_MACHINE}))
        local l_num_nodes=${#ALL_NODES[@]}
        if [ ${l_num_nodes} -lt ${l_num_servs} ]
        then
            echo "" >&2
            echo "Server machine not enough" >&2
            echo "" >&2
        ...
        SERV_MACHINES=( )
        for node_index in $(seq 0 $((${l_num_servs}-1)))
        do
            SERV_MACHINES[${node_index}]=${ALL_NODES[${node_index}]}${USE_IB}
        done

        CLIENT_MACHINES=( )
        for node_index in $(seq 0 $((${l_num_nodes}-${l_num_servs}-1)))
        do
            CLIENT_MACHINES[${node_index}]=${ALL_NODES[$((${node_index}+${l_num_servs}))]}${USE_IB}
        done
